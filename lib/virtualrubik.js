...
var moves = [[3,1,0], [3,0,1], [2,1,2], [3,1,0], [3,0,1]];
var unmoves = [[3,2,1], [3,1,2], [2,1,0], [3,2,1], [3,1,2]];
var currentMove = 0;
 ...
 VirtualRubik.prototype.initScene = function() {
  ...
  // for (var i = 0; i < 5; i++) {
  //       this.transforma(this.cube3d, moves[i][0], moves[i][1], moves[i][2])
  // };
...
VirtualRubik.prototype.onMouseUp = function(event) {
  ...
  //   //setTimeout(function(){OpenInNewTab("http://localhost")}, 400);
...
VirtualRubik.prototype.solve = function() {
  this.transforma(this.cube3d, unmoves[currentMove-1][0], unmoves[currentMove-1][1], unmoves[currentMove-1][2]);
  currentMove--;
  this.draw();
}

VirtualRubik.prototype.unsolve = function() {
  this.transforma(this.cube3d, unmoves[currentMove][0], unmoves[currentMove][1], unmoves[currentMove][2]);
  currentMove++;
  this.draw();
}

VirtualRubik.prototype.transforma = function(cube3d, face, u, v) {
    var axis=cube3d.boxClickToAxisMap[face][u][v];
    var layerMask=cube3d.boxClickToLayerMap[face][u][v];
    var angle=cube3d.boxClickToAngleMap[face][u][v];
